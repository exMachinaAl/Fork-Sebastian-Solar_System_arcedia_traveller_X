...
public class PlayerController : GravityObject {
	...
	public float jetpackDuration = 10;
	...
	//##AL_cust
	public GameObject senterObj;
	bool isUseSenter = false;
	KeyCode btnSenter = KeyCode.L;
	...
	void ToggleSenter()
	{
		if (senterObj == null) return;

		isUseSenter = !isUseSenter;
		senterObj.SetActive(isUseSenter);
	}
	
	public void OnResumeGame()
	{
		// reset yaw/pitch agar sesuai kamera saat unpause
	...
	void HandleMovement () {
		...
		if (Time.timeScale == 0 || debug_playerFrozen || referenceBody == null)
			return;
		...
		Vector3 gravityUp = transform.position - referenceBody.Position;
		if (gravityUp.sqrMagnitude > 0.0001f) {
			gravityUp.Normalize();
			cam.transform.localRotation = Quaternion.Euler(smoothPitch, 0f, 0f);
			float deltaYaw = Mathf.DeltaAngle(smoothYawOld, smoothYaw);
			Quaternion yawRotation = Quaternion.AngleAxis(deltaYaw, gravityUp);
		...
		// // }
		// if (!debug_playerFrozen && Time.timeScale > 0 && referenceBody != null)
		// {
		// 	// Pitch (kamera saja)
		// 	cam.transform.localRotation = Quaternion.Euler(smoothPitch, 0f, 0f);

		// 	// Yaw diputar terhadap gravity-up
		// 	float deltaYaw = Mathf.DeltaAngle(smoothYawOld, smoothYaw);
	...
	void FixedUpdate () {
		...
		if (gravityOfNearestBody.sqrMagnitude > 0.0001f) {
			Vector3 gravityUp = -gravityOfNearestBody.normalized;
